  ...
  setup() {
    ...
    const fetchCatDetails = async () => {
      try {
        ...
        const catResponse = await http.get(`/allatok/${catId}`);
        cat.value = catResponse.data;

        // Fetch health record if exists
        try {
          const healthResponse = await http.get(`/egeszsegugy/${catId}`);
        ...
        error.value = 'Failed to load cat details';
        loading.value = false;
...
<template>
  <section class="py-5">
    <div class="container">
      ...
      <div v-else-if="cat" class="row">
        <div class="col-md-6">
          <img :src="cat.faj === 'macska' ? '../../imgs/macska.jpg' : '../../imgs/kiskutya.kicsi_.webp'"
               class="img-fluid rounded" alt="Animal" />
        </div>
        <div class="col-md-6">
          <h1 class="mb-4">{{ cat.nev }}</h1>
          <div class="mb-3">
            <strong>Faj:</strong> {{ cat.faj === 'macska' ? 'Macska' : 'Kutya' }}
          </div>
          <div class="mb-3">
            <strong>Életkor:</strong> {{ cat.eletkor }} éves
          </div>
          <div class="mb-3">
            <strong>Nem:</strong> {{ cat.nem === 'him' ? 'Hím' : 'Nőstény' }}
          </div>
          <div class="mb-3">
            <strong>Állapot:</strong> {{ cat.orokbefogadasi_statusz }}
          </div>
          ...
          <button class="btn btn-primary mt-4" v-if="cat.orokbefogadasi_statusz === 'szabad'" @click="submitAdoption">
            Örökbefogadom
          </button>
        </div>
      </div>
